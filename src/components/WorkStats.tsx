...
export default function WorkStats({ entries }: Props) {
  ...
  const stats = [
    { icon: Clock, label: "Gesamtstunden", value: `${totalHours}h ${totalMins.toString().padStart(2, "0")}m` },
    { icon: CalendarDays, label: "Arbeitstage", value: uniqueDays.toString() },
    { icon: MapPin, label: "Orte", value: uniqueLocations.toString() },
  ];

  return (
    <div className="grid grid-cols-3 gap-3">
      {stats.map((s) => (
        <div key={s.label} className="bg-card rounded-xl p-4 text-center shadow-sm border">
