CREATE OR REPLACE FUNCTION public.record_patrol_scan(_payload text, _nfc_id text, _lat double precision, _lng double precision, _session_id uuid, _route_id uuid, _scanned_at timestamp with time zone DEFAULT now())
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF length(_payload_trim) > 0 THEN
    _method := 'qr';
    IF _payload_trim NOT LIKE 'LDN1.%' THEN
      RAISE EXCEPTION 'invalid qr: unsigned payload rejected';
    END IF;
